...
name: docs-cockpit
description: |
  Build a self-contained, live HTML cockpit (single `index.html`) that aggregates a project's Markdown documents — PRD, spec, plan, RFC, task cards, design system, todos, session memory — into one `file://`-openable preview page with sidebar navigation and an optional frontmatter-driven project dashboard (KPI / module kanban / sprint timeline). Markdown is rendered client-side via marked.js + highlight.js from CDN.

  Use this skill whenever the user mentions any of: "docs dashboard", "文档看板", "项目看板", "doc cockpit", "把 MD 文档汇总成 HTML", "spec / plan / PRD 集中预览", "build docs preview", "实时同步 docs/index.html", "项目进度面板", or any similar request to bundle many MD files into a single navigable HTML page. Also trigger when the user wants to (a) bootstrap such a page for a new project, (b) add a new doc source / group / scan rule to an existing cockpit, (c) wire frontmatter status/progress fields into a kanban view, (d) change design tokens / branding for the page, or (e) author the build script that keeps `docs/index.html` in lock-step with the underlying MD files. Don't wait for the user to say the word "skill" — if the task is "scan a docs/ tree and produce a previewable HTML aggregator", this is the right tool. If the user has an existing `scripts/build_docs_html.py` (or similar) and asks to "重写 / 通用化 / 提取到独立项目", route here too.
---
