...
def cmd_build(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if refine_map:
            ...
            # 0.14.4 · per-module refine MD 文件 · 兑现 v0.11 plan §0 「mode 1 · build-time auto」
            # dashboard 干掉了 Refine 按钮 · 用户改成 `cat docs/refine/M<id>.md | claude` 类似 pipe
            # 一次性给所有 module 在 disk 上的 MD · 不再 per-module 点按钮
            refine_dir = output.parent / "refine"
            refine_dir.mkdir(exist_ok=True)
            # 清理过期 file(老 module 删了 · refine MD 该跟着消失)
            for old in refine_dir.glob("*.md"):
                if old.stem not in refine_map:
                    try:
                        old.unlink()
                    except OSError:
                        pass
            for mid, prompt_text in refine_map.items():
                (refine_dir / f"{mid}.md").write_text(prompt_text, encoding="utf-8")
            # 索引文件 · 给 batch consumer 用 · 一次列所有 refine target
            index_lines = [
                ...
                "```",
                "",
                f"Modules({len(refine_map)}):",
                "",
            ]
            for mid in sorted(refine_map):
                index_lines.append(f"- [`{mid}.md`](./{mid}.md)")
            (refine_dir / "README.md").write_text(
                "\n".join(index_lines) + "\n", encoding="utf-8"
            )
