...
- [ ] @plan-eng-review 1A · 拆 `schema.py` + `paths.py` + `cli.py` @code:docs_cockpit/build.py
- [ ] W1 · `normalize_subtasks` + `validate_subtask_schema` @docs:docs/plans/P-v0.11-driver-seat.md
- [ ] W1 · `resolve_code_anchor` + defensive IO + `@lru_cache` @docs:docs/plans/P-v0.11-driver-seat.md
- [ ] HTML template `<script type="application/json">` 嵌入策略 · `__DOCS_JSON__` collision 消除
- [ ] sidecar 输出 · `docs/prompts.js` + `docs/code_previews.js`
- [ ] `</script>` escape 防 script tag 提前关闭
- [ ] pytest 测试基础 · `tests/unit/` + `tests/integration/`
