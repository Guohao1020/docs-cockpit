...
subtasks:
  ...
  - id: M14-5fc63c
    title: "data-i18n + hardcoded fallback 扫描 · 0 alpha 残留(0.12.1 已经修 `§4.d will fill`)"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M14-ec0442
    title: "tests/integration/test_dashboard_render.py · 不依赖 playwright 的轻量 string-based check · 24 tests cover dashboard root structure + CSS hidden overrides + topbar entries + sidecar scripts + filter bar + bundle bar"
    status: done
    code: "tests/integration/test_dashboard_render.py"


  - id: M14-7f77da
    title: "M14-022d30 simplified · pytest-playwright hash route 测试留 v0.15(避免 browser binary dep 进 CI)· 当前 test_dashboard_render.py 已 cover static structure"
    status: done
    code: "tests/integration/test_dashboard_render.py"


  - id: M14-adacba
    title: "CHANGELOG 加 CSS audit section · 走 v0.14.3 patch · 列 4 元素 explicit override 验证 + 不加 global safety net 的评估"
    status: done
