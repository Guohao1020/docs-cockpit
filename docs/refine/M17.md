...
subtasks:
  ...
  - id: M17-96dc09
    title: "起 bundle 引擎核心 · 渲染聚合 prompt + 算 cohesion / conflict + 推荐执行顺序"
    status: done
    code: "docs_cockpit/bundle.py"


  - id: M17-47f583
    title: "聚合 prompt 模板 · 共享 module 上下文去重 · 列推荐顺序 · 提示串行汇报"
    status: done
    code: "docs_cockpit/templates/prompts/bundle.md.j2"


  - id: M17-b0a08d
    title: "suggest 视角的 bundle 候选模板 · 让 LLM 自动挑哪些 subtask 适合一起打包"
    status: done
    code: "docs_cockpit/templates/suggest/bundle-recommendation.md.j2"


  - id: M17-06a14d
    title: "CLI 加批量 prompt 入口 · 用户传一串 subtask id 就拿到聚合 prompt"
    status: done
    code: ["docs_cockpit/cli.py", "docs_cockpit/bundle.py:325-370"]


  - id: M17-b16abe
    title: "build 阶段把 pairwise cohesion / conflict 预算成 sidecar · 给驾驶舱多选 UI 即时 verdict"
    status: done
    code: ["docs_cockpit/build.py", "docs_cockpit/bundle.py:298-322"]


  - id: M17-0d6c6c
    title: "author skill 加 bundle 启发式章节 · 讲清 4 维 cohesion + 4 维 conflict + 推荐顺序 + 反例"
    status: done
    code: "skills/docs-cockpit-author/SKILL.md"


  - id: M17-8d79ce
    title: "单元测试覆盖 bundle 引擎全部能力 · cohesion / conflict / 顺序 / 渲染 / sidecar"
    status: done
