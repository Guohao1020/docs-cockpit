...
subtasks:
  ...
  - id: M13-a1793d
    title: "`browser_storage.py` scaffold · `find_profile_dir(name)` 跨平台 + `read_localstorage_chrome(dir)` + `read_localstorage_firefox(path)`"
    status: done



  - id: M13-2fdc44
    title: "Chrome / Edge LevelDB reader · 走 plyvel(optional dep)+ leveldb key 前缀 filter"
    status: done



  - id: M13-b8fd60
    title: "Firefox SQLite reader · 走 stdlib sqlite3 · 不加新 dep"
    status: done



  - id: M13-5ccbc2
    title: "pyproject.toml 加 `[project.optional-dependencies] browser = ["plyvel>=1.5"]`"
    status: done



  - id: M13-0150cc
    title: "sync_status.py::cmd_sync_status `--from-browser` 路径接通 · 替换 stub 报错"
    status: done



  - id: M13-771f03
    title: "`--profile <name>` 显式选 profile · default 走 macOS / Windows / Linux per-platform default-release"
    status: done



  - id: M13-b23523
    title: "tests/unit/test_browser_storage.py · fixture LevelDB(从 string dict 构造)+ fixture SQLite"
    status: done



  - id: M13-5814c9
    title: "references/sync_status_workflow.md 更新 Path 2 章节 · 标 v0.13+"
    status: done
