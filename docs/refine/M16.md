你正在 **refine** docs-cockpit module **M16 · Multi-subtask bundle selection · backlog UX**(sprint 0.14)。
...
subtasks:
  ...
  - id: M16-4f4f32
    title: "Backlog 每行 checkbox + data-st-key · click `_toggleBundleSelection(stKey, checked)`"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M16-9fb99a
    title: "localStorage `BUNDLE_SELECTION_KEY` · 跟 0.11.3 build-time invalidation 一致(build 切就 reset)"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M16-f3ed2b
    title: "`<aside id="bundle-bar" hidden>` · fixed bottom · HP 蓝 [Copy bundle prompt] 按钮 + outline [Clear]"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M16-2e93e4
    title: "`_renderBundleBar()` · N≥1 显 / 走 `window.__BUNDLE_META__` 算 cohesion verdict 显在 verdict span"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M16-a65759
    title: "[Clear] 清 selection · re-render backlog 清 checked 状态"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M16-b6fc33
    title: "[Copy bundle prompt] · 走 CLI command copy(MVP · 复制 `docs-cockpit prompt --bundle <ids> --copy` 命令到剪贴板)· 用户终端跑一次拿真 prompt"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M16-b7a5cc
    title: "i18n EN/中 · bundle.copy / bundle.clear / toast.bundle_cli_copied"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"


  - id: M16-7ebac7
    title: "[Copy bundle prompt] 状态 · 当 `window.__BUNDLE_META__` 没加载 verdict span 显 fallback message"
    status: done
    code: "docs_cockpit/templates/index.html.tmpl"
...
`docs/spec/module/M15-backlog-view-filters.md`
